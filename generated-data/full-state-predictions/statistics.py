...
def main(start, stop):
    ...
    for d in range(start, stop):
        ...
        try:
            with open("results/networks-"+str(d)+".pickle", "rb") as f:
                data = pickle.load(f)
                G0 = data["Gactual"]
                G2 = data["Gpredict"]
                Gdiff = data["Gdiff"]

            with open("results/walks-actual-"+str(d)+".pickle", "rb") as f:
                data = pickle.load(f)
                seq = data["seq"]
                target = data["target"]
                temp = data["temp"]
    ...
    with open("results/statistics-output.pickle", "wb") as f:
    ...
    with open("results/binary-input.pickle", "wb") as f:
        pickle.dump(out,f)
